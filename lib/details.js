...
const code = json => h('pre.mono.bg-grey.p-2.small', pretty(json))
...
const paramsTable = resp =>  {
  ...
  return h('table.fullWidth', R.map(a => 
    h('tr', [
      h('td.py-2.px-1.mono.small', String(a[0]))
    , h('td.py-2.px-1', [
        String(a[1])
...
const simpleRow = arr =>
  h('tr', [
    h('td.px-1.py-2.mono.small', String(arr[0]))
  , h('td.px-1.py-2', String(arr[1]))
  ])

const simpleTable = obj => {
  if(isEmpty(obj)) return none() 
  return h('table.width-full', R.map(a => 
    simpleRow(a)
  , R.toPairs(obj)))
}

const landingPage = introMD =>
  h('article.pl-2.col-right.pb-3', [
    h('p.m-0.p-2.bg-blue--light', 'Welcome to the CommitChange API')
...
module.exports = state  => {
  ...
  return h('article.pl-2.col-right.pb-3', [
    h('h4.mono.m-0.bg-blue--light.p-2', 
